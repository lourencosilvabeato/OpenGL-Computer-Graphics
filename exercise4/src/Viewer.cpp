...
void Viewer::CreateGeometry()
{
	...
	mOffsets.clear();
	mOffsets.push_back(Eigen::Vector2f(0.0f, 0.0f));
	offsetBuffer.uploadData(mOffsets);
	...
	for (int z = 0; z < PATCH_SIZE - 1; z++)
	{
		if (z > 0)
		{
			indices.push_back(z * PATCH_SIZE);
		}

		for (int x = 0; x < PATCH_SIZE; ++x)
		{
			uint32_t i0 = z * PATCH_SIZE + x;
			uint32_t i1 = (z + 1) * PATCH_SIZE + x;
	...
	offsets.push_back(Eigen::Vector2f(0.0f, 0.0f));
	...
	glVertexAttribPointer(offsetAttrib, 2, GL_FLOAT, GL_FALSE, sizeof(Eigen::Vector2f), (void *)0);
...
void Viewer::UpdateVisiblePatches()
{
	Eigen::Vector4f frustumPlanes[6];
	nse::math::BoundingBox<float, 3> viewBBox;
	CalculateViewFrustum(proj * view, frustumPlanes, viewBBox);

	Eigen::Vector3f camPos = view.inverse().col(3).head<3>();
	int camPatchX = static_cast<int>(std::floor(camPos.x() / PATCH_SIZE));
	int camPatchZ = static_cast<int>(std::floor(camPos.z() / PATCH_SIZE));

	const int RANGE = 2;

	mOffsets.clear();
	int visiblePatchesCount = 0;

	for (int dz = -RANGE; dz <= RANGE; ++dz)
	{
		for (int dx = -RANGE; dx <= RANGE; ++dx)
		{
			int patchX = camPatchX + dx;
			int patchZ = camPatchZ + dz;

			Eigen::Vector3f patchMin(patchX * PATCH_SIZE, 0, patchZ * PATCH_SIZE);
			Eigen::Vector3f patchMax(patchX * PATCH_SIZE + PATCH_SIZE - 1, 15, patchZ * PATCH_SIZE + PATCH_SIZE - 1);
			...
			if (visible)
			{
				mOffsets.push_back(Eigen::Vector2f(patchX * PATCH_SIZE, patchZ * PATCH_SIZE));
				visiblePatchesCount++;
			}
		}
	}
	...
	this->visiblePatches = visiblePatchesCount;
}
...
void Viewer::drawContents()
{
	...
	Eigen::Matrix4f mvp = (proj * view).eval();
	...
	terrainShader.setUniform("roadNormalMap", 5);

	GLsizei indexCount = (2 * PATCH_SIZE) * (PATCH_SIZE - 1) + 2 * (PATCH_SIZE - 2);

	GLsizei numInstances = static_cast<GLsizei>(mOffsets.size());
	glDrawElementsInstanced(GL_TRIANGLE_STRIP, indexCount, GL_UNSIGNED_INT, 0, numInstances);

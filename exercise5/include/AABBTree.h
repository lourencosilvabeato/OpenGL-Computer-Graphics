...
public:
	...
	ResultEntry ClosestPrimitive(const Eigen::Vector3f &q) const
	{
		...
		const Primitive *bestPrimitive = nullptr;

		std::priority_queue<SearchEntry> pq;
		pq.push(SearchEntry(0.0f, root));

		while (!pq.empty())
		{
			SearchEntry e = pq.top();
			pq.pop();

			if (e.sqrDistance > bestDistance)
				continue;

			if (e.node->IsLeaf())
			{
				AABBLeafNode *leaf = (AABBLeafNode *)e.node;
			...
			{
				AABBSplitNode *split = (AABBSplitNode *)e.node;
				float d1 = split->Left()->GetBounds().SqrDistance(q);
				float d2 = split->Right()->GetBounds().SqrDistance(q);
				if (d1 < d2)
				{
					pq.push(SearchEntry(d1, split->Left()));
					pq.push(SearchEntry(d2, split->Right()));
				}
				else
				{
					pq.push(SearchEntry(d2, split->Right()));
		...
		return ResultEntry(bestDistance, bestPrimitive);
	}

...
void Viewer::CreateVertexBuffers()
{
	...
	GLuint vid_color = glGetAttribLocation(program_id, "in_color");

	glEnableVertexAttribArray(vid_color);
	glVertexAttribPointer(vid_color, 4, GL_FLOAT, GL_FALSE, 0, 0);
	...
	GLuint fractalcoord_buffer_id;
	glGenBuffers(1, &fractalcoord_buffer_id);
	glBindBuffer(GL_ARRAY_BUFFER, fractalcoord_buffer_id);
	glBufferData(GL_ARRAY_BUFFER, sizeof(fractalCoords), fractalCoords, GL_STATIC_DRAW);

	GLint fractal_loc = glGetAttribLocation(program_id, "in_fractalCoord");

	glEnableVertexAttribArray(fractal_loc);
...
void Viewer::drawContents()
{
	...
	*/
	...
	GLint locModelView = glGetUniformLocation(program_id, "modelViewMatrix");
	GLint locProjection = glGetUniformLocation(program_id, "projectionMatrix");

	glUniformMatrix4fv(locModelView, 1, GL_FALSE, modelViewMatrix.data());
	glUniformMatrix4fv(locProjection, 1, GL_FALSE, projectionMatrix.data());

	// Bind the vertex array
	glBindVertexArray(vertex_array_id);

	GLint locJuliaC = glGetUniformLocation(program_id, "juliaC");
	GLint locJuliaZoom = glGetUniformLocation(program_id, "juliaZoom");

	glUniform2f(locJuliaC, juliaC.x(), juliaC.y());
	glUniform1f(locJuliaZoom, juliaZoom);
